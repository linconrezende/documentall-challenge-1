    ...
    <v-row>
      ...
      <v-col cols="12" v-for="(file, index) in files" :key="index">
        <p class="display-1 text-center">
          Encontramos {{ countValidMessages(file.phoneMessages) }} mensagens
          válidas em sua lista &#127881;
        </p>
        <p class="body-1">
          <b>File name: {{ file.name }}</b>
        </p>
        <template>
          <v-data-table
            dense
            ...
            <template v-slot:[`item.valid`]="{ item }">
              <div>
                <span v-if="item.valid"
                  >Válida
                  <v-icon class="green--text ml-2">fas fa-check</v-icon></span
                >
                <div v-if="!item.valid">
                  <v-tooltip v-if="item.invalidReasons" bottom>
                    ...
                    <span>
                      <p
                        v-for="(reason, ixinvrs) in item.invalidReasons"
                        :key="ixinvrs"
                        class="pa-0 ma-0"
                      >
                        {{ reason }}
                  ...
                  <span v-else
                    >Inválida
                    <v-icon class="red--text ml-2">fas fa-times</v-icon></span
                  >
                </div>
              </div>
            </template>
      ...
      <v-col cols="12" class="text-center pt-5">
        ...
        <v-btn :disabled="savingList" large @click="cancel()"
          >Cancelar<v-icon class="ml-2">fas fa-times</v-icon></v-btn
        >
        <v-btn
          :disabled="savingList"
          large
          ...
          color="primary"
          @click="saveLists()"
          >Salvar lista(s)<v-icon class="ml-2">fas fa-check</v-icon></v-btn
        >
      </v-col>
    </v-row>
  </v-container>
...
export default {
  ...
  methods: {
    ...
    async saveLists() {
      ...
      vm.$API
        .Request("POST", "phoneMessage/create", phoneMessages)
        .then((result) => {
